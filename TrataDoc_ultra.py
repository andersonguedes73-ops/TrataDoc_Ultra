        ...
        for cam in self.dados["tarja"]["entrada"]:
            try:
                with fitz.open(cam) as d:
                    for p in d:
                        txt = p.get_text("text"); termos.update(re.findall(r'\b(?:\d[\.\-\/\\]?){3,}\b', txt))
                        for ent in nlp(txt).ents:
                            if ent.label_ == "PER" and len(ent.text) > 3: termos.add(ent.text.strip())
            except Exception as e:
                self.after(0, lambda err=e: messagebox.showerror("Erro na Análise", f"Falha:\n{err}"))
        self.caixa_rev.delete("0.0", "end"); [self.caixa_rev.insert("end", f"{t}\n") for t in sorted(list(termos))]
    ...
    def tarjar(self):
        ...
        for cam in self.dados["tarja"]["entrada"]:
            try:
                ...
                self.after(0, lambda err=e: messagebox.showerror("Erro ao Tarjar", f"Falha:\n{err}"))
